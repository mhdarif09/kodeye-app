      ...
      {problem && (
        <div className="border-b border-border bg-muted/20 shrink-0">
          <button
            ...
            className="w-full flex items-center gap-2 px-4 py-2 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider hover:bg-muted/40 transition"
          >
            <svg
              className={`w-3 h-3 transition-transform ${problemOpen ? "rotate-90" : ""}`}
          ...
          {problemOpen && (
            <div className="px-4 pb-3 max-w-3xl mx-auto">
              <p className="text-sm text-foreground/90 whitespace-pre-wrap">{problem}</p>
              {problemTemplate && (
